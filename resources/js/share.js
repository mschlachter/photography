(function () {
	...
	var create_share_modal = function() {
	    ...
	    div.innerHTML = '<button class="close-btn" aria-label="close share modal"></button>' +
	    				'<div class="good-share-modal-buttons">' +
                            ...
                            '<div class="text-area"><input class="input-share-url" readonly="readonly"><div class="tooltip"><button class="btn copy-btn"><span class="tooltiptext">Copy to Clipboard</span>Copy</button></div></div>'+
 			            '</div>';
	...
	var open_share_modal = function( share_title, share_text, share_url ) {
		...
		if ( navigator.share && isSamsungBrowser === null ) {
            ...
            //fill in the copiable text box
            document.querySelector(".good-share-modal-buttons .input-share-url").value = share_url;
	...
	document.addEventListener('click', function (event) {
	    ...
	    if ( event.target.classList.contains( 'copy-btn' ) ) {
            var copyText = document.querySelector(".good-share-modal-buttons .input-share-url");
            copyText.select();
            copyText.setSelectionRange(0, 99999);
            document.execCommand("copy");

            var tooltip = document.querySelector(".good-share-modal-buttons .tooltiptext");
