...
<picture>
    ...
    <img @if($id ?? false) id="{{ $id }}" @endif class="{{ $class ?? '' }}" src="{{ $media->getUrl() }}" title="{{ $image->title }}" alt="{{ $image->alt }}" @if(isset($width)) data-width="{{ $width }}" @endif @if(isset($height)) data-height="{{ $height }}" @endif>
</picture>
